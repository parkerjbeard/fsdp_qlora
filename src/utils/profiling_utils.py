...
def trace_handler(prof: torch.profiler.profiler.profile, rank, export_memory_timeline, output_dir, metric="self_cuda_time_total", with_stack=True, group_by_stack=0, group_by_input_shape=False, row_limit=25):
    ...
    if export_memory_timeline:
        try:
            ...
            logger.info(f"Failed to export memory timeline to html: {e}, retrying as gzipped json.")
            try:
                prof.export_memory_timeline(
                    f"{curr_trace_dir}/rank{rank}_memory-timeline.json.gz"
                )
            except Exception as e2:
                logger.info(f"Failed to export memory timeline to gzipped json: {e2}. Saving profiler timeline object instead.")
                try:
                    ...
                    logger.error(f"Failed to save memory timeline in any format: {e3}")
    ...
    # Ensure all ranks have finished saving profiling data before continuing
